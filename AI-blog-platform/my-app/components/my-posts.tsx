...
export function MyPosts({ onCreatePost, onEditPost, onViewPost }: MyPostsProps) {
  ...
  const loadPosts = useCallback(async () => {
    try {
      ...
      const res = await fetch(`/api/posts/all?${params.toString()}`);
      if (res.ok) {
        const data = await res.json();
        setPosts(data.posts || []);
        setCounts(data.counts || { published: 0, draft: 0, total: 0 });
      } else {
        setPosts([]);
  ...
  const handleDelete = async (post: BlogPost) => {
    try {
      await fetchWithAuth(`/api/posts/${post.id}`, { method: "DELETE" });
